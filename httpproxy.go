...
http=%[5]s:%[6]s, username=%[8]s, password=%[9]s, over-tls=%[7]t, tls-host=%[2]s, tls-verification=%[10]t, fast-open=false, udp-relay=false, tag=%[1]s (via Server)`
	...
	switch req.URL.Path {
	case "/clash":
		fmt.Fprintf(rw, clashTemp, name, host, eip, eportStr, rip, rportStr, tls, user, pass, tlsInsecure)
	case "/quan":
		fallthrough
	case "/quanx":
		fmt.Fprintf(rw, quanxTemp, name, host, eip, eportStr, rip, rportStr, tls, user, pass, !tlsInsecure)
	default:
